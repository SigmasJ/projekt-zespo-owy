...
1. Sklonuj repozytorium:
   git clone https://github.com/SigmasJ/projekt-zespo-owy.git
2. Przygotuj bazę danych:
   - Utwórz nową bazę (np. projekt).
   - Zaimportuj skrypt:
     mysql -u <użytkownik> -p <nazwa_bazy> < czatonotatnik.sql
   - Alternatywnie użyj phpMyAdmin lub innego narzędzia do importu.
3. Skonfiguruj połączenie z bazą:
   - Otwórz db.php i zaktualizuj wartości host, użytkownik, hasło, nazwa bazy.
   - Upewnij się, że plik nie zawiera w repozytorium rzeczywistych haseł produkcyjnych.
4. Skonfiguruj JWT:
   - Sprawdź jwt.php — ustaw bezpieczny sekret (secret key). Zalecane trzymać go poza repo (np. w zmiennej środowiskowej).
5. Uruchom serwer deweloperski PHP (opcjonalnie):
   cd projekt-zespo-owy
   php -S localhost:8000
   Następnie otwórz http://localhost:8000/index.html
6. Testowanie API:
   - Użyj tester.html lub narzędzi typu Postman/curl wysyłając żądania do api.php.

Przykład importu SQL:
mysql -u root -p projekt < czatonotatnik.sql

Przykład uruchomienia lokalnie z PHP built-in:
php -S 0.0.0.0:8000
...
## Co można zrobić dalej (pomysły rozwoju)
- Dodać plik konfiguracyjny .env i mechanizm wczytywania zmiennych środowiskowych.
- Rozdzielić logikę w api.php na moduły w katalogu modules/.
- Dodać instrukcję uruchomienia z Docker (Dockerfile + docker-compose) dla szybkiego środowiska deweloperskiego.
- Dodać testy automatyczne i przykładowe scenariusze API.

## Licencja
W repo nie znaleziono pliku LICENSE. Jeżeli projekt ma być publiczny, dodaj proszę odpowiednią licencję.

---

Co zrobiłem: przejrzałem listę plików i przygotowałem powyższy README, które ma za zadanie pomóc w szybkim zrozumieniu projektu i uruchomieniu go lokalnie.

Co dalej: jeśli chcesz, mogę
- wygenerować gotowy plik README.md i zaproponować commit,
- przygotować Dockerfile + docker-compose do łatwego uruchomienia,
- lub przejrzeć zawartość wybranych plików (np. jwt.php, db.php, api.php) i zaproponować poprawki bezpieczeństwa.
Wybierz, co mam zrobić następnie.
